...
export class BuildingEngine{
 ...
 async build(seed,lotX=this.lotX,lotZ=this.lotZ,resident=true){if(![lotX,lotZ].every(v=>Number.isInteger(v)&&Math.abs(v)<=1000000))throw Error("Lot coordinates must be integers between -1000000 and 1000000.");if(!Number.isInteger(seed)||seed<0||seed>16777215)throw Error('Seed must be an integer from 0 to 16777215.');await this.runtime.idle();this.seed=seed;this.lotX=lotX;this.lotZ=lotZ;this.resident=resident;this.rebuilds++;
  ...
  this.runtime.batch().dispatch(this.bind('generateScene',{seed,lotX,lotZ,resident:resident?1:0,radius:this.radius,cacheEnabled:this.cacheEnabled?1:0,virtualEnabled:this.parallaxEnabled?1:0}),[1]).dispatch(this.bind('describeLayout'),[1]).submit();
 ...
 async stream(camera=null){
  ...
  let resident=(dx!==0||dz!==0||this.hasBuilding)&&c[1]<20&&distance<(this.resident&&!dx&&!dz?11:8);
  if(this.parallaxEnabled){this.runtime.batch().dispatch(this.bind('probeResidency'),[1]).submit();resident=(await this.runtime.read(this.buffers.Plan,Float32Array,4,46*4))[0]>.5;}
 ...
 async setFly(enabled){if(enabled===this.flying)return;if(!enabled){await this.stream();if(!this.resident&&!this.parallaxEnabled)await this.build(this.seed,this.lotX,this.lotZ,true);const c=await this.camera();c[5]=0;c[6]=0;this.setCamera(c);}this.flying=enabled;}
 ...
 async view(view){if(view!==1&&!this.resident&&this.hasBuilding)await this.build(this.seed,this.lotX,this.lotZ,true);this.runtime.batch().dispatch(this.bind('initCamera',{view}),[1]).submit();await this.runtime.idle();this.sample=0;}
