...
function describe(){ $('floor-plan').textContent=engine.hasBuilding?['Central hall · Rear stairs','Open living · Left hall','Open living · Right hall','Front stairs · Side entrance'][engine.layout[64]]:''; $('economy').textContent=`${engine.hasBuilding?'Home':'Land'} value · ${Math.round(engine.layout[15]*100)} / 100 · District ${'$'.repeat(engine.layout[14]<.35?1:engine.layout[14]>.65?3:2)}`; $('lot-x').value=engine.lotX;$('lot-z').value=engine.lotZ;$('building-kind').textContent=engine.hasBuilding?`${engine.layout[2]} FLOORS · ${['RESIDENTIAL','WORKSPACE','MIXED USE'][engine.layout[4]]}`:'DISTRICT OPEN SPACE';$('residency').textContent=!engine.hasBuilding?`Open land · ${(engine.radius*2+1)**2} plots cached`:engine.resident?`${engine.layout[68]} rooms resident · ${(engine.radius*2+1)**2} cached`:`Virtual interiors · ${(engine.radius*2+1)**2} cached`;}
try{
 await engine.init(message=>$('loading').textContent=message);if(params.has('seed')){await engine.build(Number(params.get('seed')),0,0,false);await engine.view(1);}$('seed').value=engine.seed;describe();
 ...
 async function enter(){if(busy)return;controls.enter();if(!entered){busy=true;try{await engine.view(0);describe();entered=true;document.body.classList.add('walking');}finally{busy=false;}}}
 ...
 $('regenerate').onclick=async()=>{if(busy)return;busy=true;$('status').textContent='GENERATING';try{await engine.build(Number($('seed').value),Number($('lot-x').value),Number($('lot-z').value),false);await engine.view(entered?0:1);describe();$('status').textContent='READY';}catch(e){fail(e);}finally{busy=false;}};
 $('entrance').onclick=async()=>{if(busy)return;busy=true;try{await engine.view(0);describe();entered=true;document.body.classList.add('walking');}catch(e){fail(e);}finally{busy=false;}};
