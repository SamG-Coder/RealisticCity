...
try{
 ...
 $('fly').onclick=async()=>{if(busy)return;busy=true;try{await engine.setFly(!engine.flying);$('fly').textContent=engine.flying?'Fly mode · F → Entrance':'Walk mode · F → Fly';$('speed').textContent=engine.flying?engine.flySpeed.toFixed(1)+' m/s · wheel adjusts':'Gravity and collisions';describe();}catch(e){fail(e);}finally{busy=false;}};
 ...
 async function frame(time){if(paused)return;try{const dt=Math.min(.10,(time-previous)/1000);previous=time;if(!busy){if(controls.active){acc+=dt;const steps=Math.min(12,Math.floor(acc*120));acc-=steps/120;engine.step(controls.input(steps),steps);}else acc=0;engine.draw();await engine.runtime.idle();fps=fps*.92+(1/Math.max(.001,dt))*.08;if(time-lastHud>200){lastHud=time;if(entered&&await engine.stream())describe();const c=await engine.camera();let floor=1;for(let f=1;f<engine.layout[2];f++)if(c[1]+.05>=engine.layout[8+f])floor=f+1;$('floor').textContent=engine.flying?`Flying · ${Math.round(c[1])} m`:entered?`Floor ${floor} / ${engine.layout[2]}`:'Exterior';$('fps').textContent=`${Math.round(fps)} fps`;}}requestAnimationFrame(frame);}catch(e){fail(e);}}
