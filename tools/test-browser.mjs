...
try{browser=await chromium.launch({channel:'msedge',headless:true,args:['--enable-unsafe-webgpu']});const page=await browser.newPage({viewport:{width:1440,height:900}});page.on('console',m=>{if(m.type()==='error')console.error(m.text());});page.on('pageerror',e=>console.error(e));
 await page.goto(`http://127.0.0.1:${server.address().port}/?test`);await page.waitForFunction(()=>window.__ready||window.__error,null,{timeout:240000});const error=await page.evaluate(()=>window.__error);if(error)throw Error(error);
 ...
 const route=await page.evaluate(async()=>{
  ...
  function world(x,z,interior=true){const p=e.layout,h=p[65],a=p[13],c=Math.cos(a),v=Math.sin(a);if(interior){x/=p[0];z/=p[1];x=(x< -1.8?-9+(x+9)*(7.2+h)/7.2:x>1.8?9-(9-x)*(7.2-h)/7.2:x+h)*p[0];z=(p[67]?3-z:z)*p[1];}return [c*x+v*z,-v*x+c*z];}
  const entrance=()=>world(e.layout[66]*e.layout[0],-14*e.layout[1],false);
  async function enterRoute(){if(e.layout[67]){await walk(6,10);await walk(6,4.8);await walk(0,4.8);}}
  async function walk(x,z){[x,z]=world(x*sx,z*sz);for(let i=0;i<500;i++){const c=await e.camera(),dx=x-c[0],dz=z-c[2];if(dx*dx+dz*dz<.012){targets++;return;}c[3]=Math.atan2(dx,dz);e.setCamera(c);e.step([1,0,0,0,0,0],4);}throw Error(`Blocked toward ${x},${z}: ${await e.camera()}`);}
  await e.view(0);e.step([],120);let c=await e.camera();if(Math.abs(c[1])>.03)throw Error('Grounding failed');
  async function rooms(f){for(let r=0;r<4;r++){if(!e.layout[80+(f*4+r)*8])continue;const z=e.layout[16+f*4+r]/sz;await walk(0,z);await walk((r%2?1:-1)*3.1,z);await walk(0,z);}}
  await enterRoute();await rooms(0);
  ...
  if(e.layout[67]){await walk(0,4.8);await walk(6,4.8);await walk(6,12);}else await walk(0,-12);e.step([0,0,0,0,0,1],15);c=await e.camera();if(c[1]<.1)throw Error('Jump failed');e.step([],180);if(Math.abs((await e.camera())[1])>.03)throw Error('Landing failed');
  const wall=world((e.layout[66]>0?-6:6)*sx,-10*sz,false);e.setCamera([wall[0],0,wall[1],e.layout[13],0,0,1,0,...Array(8).fill(0)]);e.step([1,0,0,0,1,0],240);c=await e.camera();if(Math.sin(e.layout[13])*c[0]+Math.cos(e.layout[13])*c[2]>-8.26*sz-.25)throw Error('Facade penetration');
  ...
  const homeYaw=e.layout[13],homeEntrance=entrance();await e.build(240921,1,0,false);const neighbourYaw=e.layout[13],neighbourEntrance=entrance();await e.build(240921,0,0,true);
  ...
  const variants=[];for(const [x,z]of [[1,0],[-2,3],[7,-4]]){
    await e.build(240921,x,z);sx=e.layout[0];sz=e.layout[1];floors=e.layout[2];variants.push({x,z,floors,roof:e.layout[3],use:e.layout[4],rooms:e.layout[68]});await e.view(0);e.step([],120);await enterRoute();await rooms(0);
  ...
  return {targets,variants,exteriorConsistency:true,rooms:e.layout[68]+variants.reduce((n,v)=>n+v.rooms,0),camera:c,errors:e.errors,streamingEviction:true,streamingReturn:true,boundedAllocation:true,seedRegeneration:true,seedVariation:true,accumulation:true};
 ...
 await page.close();
 const ui=await browser.newPage({viewport:{width:1366,height:768}});await ui.goto(`http://127.0.0.1:${server.address().port}/?resolution=1280`);await ui.waitForFunction(()=>window.__ready||window.__error,null,{timeout:240000});if(await ui.evaluate(()=>window.__error))throw Error(await ui.evaluate(()=>window.__error));await ui.locator('#enter').click();await ui.waitForFunction(()=>document.pointerLockElement!==null&&document.body.classList.contains('walking'));await ui.keyboard.down('KeyW');let moved;for(let attempt=0;attempt<50;attempt++){moved=await ui.evaluate(()=>engine.camera());if(moved[7]>.3)break;await ui.waitForTimeout(100);}await ui.keyboard.up('KeyW');if(!(moved[7]>.3))throw Error('Actual WASD input failed: '+JSON.stringify(moved));await ui.keyboard.press('Escape');await ui.waitForFunction(()=>document.pointerLockElement===null);await ui.locator('#resolution').selectOption('2560');await ui.waitForFunction(()=>engine.width===2560);await new Promise(r=>setTimeout(r,400));if(await ui.evaluate(()=>window.__error))throw Error(await ui.evaluate(()=>window.__error));
