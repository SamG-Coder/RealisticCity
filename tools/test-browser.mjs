...
try{browser=await chromium.launch({channel:'msedge',headless:true,args:['--enable-unsafe-webgpu']});const page=await browser.newPage({viewport:{width:1440,height:900}});page.on('console',m=>{if(m.type()==='error')console.error(m.text());});page.on('pageerror',e=>console.error(e));
 ...
 const route=await page.evaluate(async()=>{
  ...
  function world(x,z){const a=e.layout[13],c=Math.cos(a),v=Math.sin(a);return [c*x+v*z,-v*x+c*z];}
  ...
  const wall=world(4*sx,-10*sz);e.setCamera([wall[0],0,wall[1],e.layout[13],0,0,1,0,...Array(8).fill(0)]);e.step([1,0,0,0,1,0],240);c=await e.camera();if(Math.sin(e.layout[13])*c[0]+Math.cos(e.layout[13])*c[2]>-8.26*sz-.25)throw Error('Facade penetration');
